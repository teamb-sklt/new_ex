...
router.post('/', async function(req,response,next){
    ...
    const sql = "UPDATE table_name SET (emp_no, sheet_year, sheet_month, branch_no, year, month, day, trans_type, trans_from, trans_to, trans_waypoint, amount, count, job_no, job_manager, claim_flag, charge_flag, ref_no, status, remarks, new, new_date, renew, renew_date) = ('"+dEmpno+"', sheet_year='"+year+"', sheet_month='"+tmonth+"', branch_no='"+branch_no2+"', year='"+dYear+"', month='"+dMonth+"', day='"+dDay+"', trans_type='"+dWay+"', trans_from='"+dStart+"', trans_to='"+dGoal+"', trans_waypoint='"+dWaypoint+"', amount='"+dPrice+"', count='"+dTimes+"', job_no='"+dJobno+"', job_manager='"+dJobmanager+"', claim_flag='"+dClaimflag+"', charge_flag='"+dChargeflag+"', ref_no='"+dRefno+"', status='"+dStasus+"', remarks='"+dMemo+"', new='"+dNew+"', new_date='"+dNewdate+"', renew='"+dNew+"', renew_date='"+dNewdate+"') WHERE sheet_month='"+tmonth+"'AND branch_no='"+branch_no2+"'";
    console.log(sql)
