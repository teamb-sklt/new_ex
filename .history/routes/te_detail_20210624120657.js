...
router.post('/', async function(req,response,next){
    ...
    //インサートコマンドを定義
    const sql = "UPDATE TeDetail SET emp_no=dEmpno, sheet_year=year, sheet_month=tmonth, branch_no=branch_no2, year=dYear, month=dMonth, day=dDay, trans_type=dWay, trans_from=dStart, trans_to=dGoal, trans_waypoint=dPrice, amount=dJobno, count=dJobmanager, job_no=dClaimflag, job_manager=dChargeflag, claim_flag=dRefno, charge_flag=dStasus, ref_no=dMemo, status= , remarks=dMemo, new=dNew, new_date=dNewdate, renew=dNew, renew_date=dNewdate WHERE sheet_month="+"'"+tmonth+"'";
