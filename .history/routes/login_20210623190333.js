var express = require('express');
var router = express.Router();
var {Client}=require('pg');
const moment = require("moment");

require('dotenv').config();
const user=process.env.USER;
const dbpassword=process.env.PASSWORD;

var today = moment();
var year = moment().format("YYYY");
var tomonth = moment().format("MM");
// var nm = moment().add(1,'month').format("MM");　//翌月
// var lm = moment().add(-1,'month',).format("MM");　//先月
var date = moment().format("DD")
var tmonth;
var lmonth;
var lastday;

console.log(date)

if(date>20){
  tmonth=moment().add(1,'month').format("MM");
  lmonth=tomonth;
}else{
  tmonth=tomonth;
  lmonth=moment().add(-1,'month',).format("MM");
  lastday = 20 - date;
}
console.log(tmonth)
var status = [];
var branch_no = [];
var month = [];
var day = [];
var trans_from = [];
var trans_to = [];
var amount = [];
var count = [];
var subtotal = [];
var job_no = [];
var emp_no = [];


/* GET home page. */
router.get('/',function(req,res,next){
    console.log(res);
    res.render('login',{
        title:'Login',
    });
});


router.post('/',async function(req, res, next) {
  const client = (process.env.ENVIRONMENT == "LIVE") ? new Client({
    connectionString: process.env.DATABASE_URL,
    ssl: {
        rejectUnauthorized: false
    }
  }) : new Client({
    user: 'postgres',
    host: 'localhost',
    database: 'itpjph3',
    password: dbpassword,
    port: 5432
  })
  let account=req.body.account;
  let pass=req.body.password;
  console.log(account+pass);

  await client.connect()
  console.log(client)
  client.query("SELECT * from Employee where emp_no='"+account+"'", function(err, result){
    if (err){
      console.log(err) //show error infomation
    }
    let record = result.rows
    for(let i = 0; i <= result.rows; i++){
    record.push(result.rows[i])
  }
    if(result.rows[0].pw==pass){	//いったん全員が入れるように設定（必要であればif()の中に&&で条件を追加してください。）
        // let id=result.rows[0].emp_no;
        // let name=result.rows[0].emp_name;
        let a=result.rows[0];
        async const client = (process.env.ENVIRONMENT == "LIVE") ? new Client({
          connectionString: process.env.DATABASE_URL,
          ssl: {
              rejectUnauthorized: false
          }
        }) : new Client({
          user: 'postgres',
          host: 'localhost',
          database: 'itpjph3',
          password: dbpassword,
          port: 5432
        })
        await client.connect()
        client.query("SELECT * from TeDetail WHERE sheet_month="+"'"+tmonth+"'" +"ORDER BY branch_no ASC",function(err,result1){
          console.log(result1)
          // console.log(result)
          for(var i in result1.rows){
            status[i]=result1.rows[i].status;
            branch_no[i]=result1.rows[i].branch_no;
            month[i]=result1.rows[i].month;
            day[i]=result1.rows[i].day;
            trans_from[i]=result1.rows[i].trans_from;
            trans_to[i]=result1.rows[i].trans_to;
            amount[i]=result1.rows[i].amount;
            count[i]=result1.rows[i].count;
            subtotal[i]=result1.rows[i].count * result.rows[i].amount;
            job_no[i]=result1.rows[i].job_no;
      
            // console.log(i)                  
          }
          client.end()
        });
        let opt={
          title: '交通費',
          emp_no:emp_no,
          tmonth:tmonth,
          lmonth:lmonth,
          lastday:lastday,
          date:date,
          status:status,
          branch_no:branch_no,
          month:month,
          day:day,
          trans_from:trans_from,
          trans_to:trans_to,
          amount:amount,
          count:count,
          subtotal:subtotal,
          job_no:job_no,
      }
        res.render('te_thismonth', opt);
        client.end();
        //res.redirect('/te_thismonth')    //成功時の遷移先
    }else{
      client.end();
      res.redirect('/login')  //失敗時の遷移先
    }
  });
});

router.post('/', async function(req, res, next) {
  let emp_no = req.body.account;
    const client = (process.env.ENVIRONMENT == "LIVE") ? new Client({
      connectionString: process.env.DATABASE_URL,
      ssl: {
          rejectUnauthorized: false
      }
    }) : new Client({
      user: 'postgres',
      host: 'localhost',
      database: 'itpjph3',
      password: dbpassword,
      port: 5432
    })
    await client.connect()
    // console.log(client)
    // client.query('SELECT * from example', function(err, result){
    //   if (err){
    //     console.log(err) //show error infomation
    //   }
    //   console.log(result)
    // })
    client.query("SELECT * from TeDetail WHERE sheet_month="+"'"+tmonth+"'" +"ORDER BY branch_no ASC",function(err,result){
      // console.log(result)
      for(var i in result.rows){
        status[i]=result.rows[i].status;
        branch_no[i]=result.rows[i].branch_no;
        month[i]=result.rows[i].month;
        day[i]=result.rows[i].day;
        trans_from[i]=result.rows[i].trans_from;
        trans_to[i]=result.rows[i].trans_to;
        amount[i]=result.rows[i].amount;
        count[i]=result.rows[i].count;
        subtotal[i]=result.rows[i].count * result.rows[i].amount;
        job_no[i]=result.rows[i].job_no;
  
        // console.log(i)                  
      }
      client.end()
    });
    let opt={
      title: '交通費',
      emp_no:emp_no,
      tmonth:tmonth,
      lmonth:lmonth,
      lastday:lastday,
      date:date,
      status:status,
      branch_no:branch_no,
      month:month,
      day:day,
      trans_from:trans_from,
      trans_to:trans_to,
      amount:amount,
      count:count,
      subtotal:subtotal,
      job_no:job_no,
  }
    res.render('te_thismonth', opt);
});

module.exports = router;