...
router.post('/', async function(req, res, next) {
  ...
  //インサートコマンドを定義
  const sql = "UPDATE TeDetail SET (emp_no, sheet_year, sheet_month, branch_no, year, month, day, code_name, payee, summary, amount, job_no, job_manager, claim_flag, charge_flag, ref_no, status, remarks, new, new_date, renew, renew_date) = ('"+dEmpno+"', '"+year+"', '"+tmonth+"', '"+branch_no2+"', '"+dYear+"', '"+dMonth+"', '"+dDay+"', '"+dCodename+"', '"+dPayee+"', '"+dGoal+"', '"+dWaypoint+"', '"+dPrice+"', '"+dTimes+"', '"+dJobno+"', '"+dJobmanager+"', '"+dClaimflag+"', '"+dChargeflag+"', '"+dRefno+"', '"+dStasus+"', '"+dMemo+"', '"+dNew+"', '"+dNewdate+"', '"+dNew+"', '"+dNewdate+"') WHERE sheet_month='"+tmonth+"'AND branch_no='"+branch_no2+"'";
  console.log(sql)
