...
router.post('/', async function(req,response,next){
    ...
    //インサートコマンドを定義
    const sql = "UPDATE TeDetail SET emp_no=dEmpno, sheet_year=year, sheet_month=tmonth, branch_no=branch_no2, year=dYear, month=dMonth, day=dDay, code_name=dCodename, payee=dPayee, summary=dSummary, amount=dPrice, job_no=dJobno, job_manager=dJobmanager, claim_flag=dClaimflag, charge_flag=dChargeflag, ref_no=dRefno, status=dStasus, remarks=dMemo, new=dNew, new_date=dNewdate, renew=dNew, renew_date=dNewdate WHERE sheet_month="+"'"+tmonth+"'";
